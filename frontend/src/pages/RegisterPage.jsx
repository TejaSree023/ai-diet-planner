...
const RegisterPage = () => {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      if (/email already|already registered|already exists/i.test(serverMessage)) {
      ...
      <section className="auth-shell">
        ...
        <div className="auth-panel auth-form-panel">
          ...
          <form onSubmit={handleSubmit} className="space-y-3">
            <input
              className={`input ${errors.name ? "!border-rose-300 !bg-rose-50" : ""}`}
              name="name"
              placeholder="Name"
              value={form.name}
            ...
            />
            <input
              className={`input ${errors.email ? "!border-rose-300 !bg-rose-50" : ""}`}
              name="email"
              type="email"
              placeholder="Email"
              value={form.email}
            ...
            <div className="rounded-2xl border border-[#e7dccf] bg-[#fffaf3] p-3">
              <p className="mb-2 text-xs font-semibold uppercase tracking-[0.14em] text-[#8C6A54]">
                Quick Onboarding
              </p>
          ...
          {(errors.name || errors.email || errors.password) && (
            <p className="mt-2 text-sm text-rose-500">{errors.name || errors.email || errors.password}</p>
          )}
          {message && <p className="mt-3 text-sm text-rose-500">{message}</p>}

          <p className="mt-3 text-sm text-[#6B665E]">
            Already have an account?{" "}
            <Link to="/login" className="font-semibold text-[#2a9f61]">
              Login
            </Link>
          </p>
        </div>
